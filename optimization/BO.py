...
try:
    ...
    # ------------------------------------------------------
    # basic definitions (atom lists, feature order, atomic property tables)
    # ------------------------------------------------------
    MAGNETIC_ATOMS = [
    ...
    # the feature order the model was trained with
    FEATURE_ORDER = [
    ...
    def get_element_symbol(z):
        ...
        # the supplied dicts (keys are strings, so they are converted to int before use)
    ...
    # atomic property tables (the abbreviated parts are taken from the original file unchanged)
    ATOM_PROPERTIES = {}

    # 1) metals (candidate magnetic atoms)
    for z_str, d_e in d_orbit_e.items():
        ...
        ATOM_PROPERTIES[z] = {
            ...
            'd_orb_e': d_e,
            'proxy_M_magnet': proxy_map.get(z_str, 3),  # default of about 3
        }

    # 2) non-magnetic p-block elements
    for z_str, p_e in non_mag_p_orbit_e.items():
        ...
        props.update({
            ...
            # add ZVAL here as well if needed
    ...
    # ------------------------------------------------------
    # derive the Optuna search ranges automatically from fin_data.csv
    # ------------------------------------------------------
    FIN_DATA_STATS = {}
    ...
    def suggest_from_stats(trial, name, default_low, default_high, log=False):
        """Prefer the value range taken from fin_data.csv; fall back to the default range."""
    ...
    # ------------------------------------------------------
    # helper functions for geometry and motifs
    # ------------------------------------------------------
    try:
    ...
    def compute_nonmag_vertex_angle_stats(coords):
        """Statistics of the ligand-ligand-ligand vertex angles (same definition as BOND_df)."""
    ...
    def compute_motif_s_delta(coords, eps=1e-12):
        """
        Compute s and delta (the same definition used in ELONG_df).
        - coords: ligand positions relative to the magnetic centre, taken as the origin
        """
        P = np.asarray(coords, dtype=float)
        if P.shape[0] < 2:
            return 0.0, 0.0
    ...
    def compute_motif_measure(coords, polyhedron):
        """
        Characteristic size of the motif (2D area or 3D volume), computed with ConvexHull.
        - square_planar : 2D area (convex hull of the in-plane projection)
        - otherwise (tetrahedral, octahedral) : 3D volume
        """
        P = np.asarray(coords, dtype=float)
        ...
        try:
            if polyhedron == 'square_planar':
                # 2D: project onto the best-fit plane and take the convex-hull area
                center = P.mean(axis=0)
                centered = P - center
                U, S, Vt = np.linalg.svd(centered, full_matrices=False)
                plane_basis = Vt[:2, :]   # the first two principal components
    ...
    # ------------------------------------------------------
    # fallback objective: features computed from explicit coordinates
    # ------------------------------------------------------
    def objective(trial, model):
        """Objective function with actual coordinate optimization and atom properties"""
        # choose 4- or 6-fold coordination
        ...
        # for 4-fold coordination both tetrahedral and square_planar are possible
        if n_nonmag_atoms == 4:
            polyhedron = trial.suggest_categorical('polyhedron', ['tetrahedral', 'square_planar'])
        else:
            polyhedron = 'octahedral'
        
        # --- generate the ligand coordinates (the magnetic atom is at the origin) ---
        coords = []
        if n_nonmag_atoms == 4 and polyhedron == 'tetrahedral':
            ...
            for i in range(4):
                ...
                if i == 0:
                    ...
                    # approximate range of tetrahedral angles
            ...
            # square planar: place four points close to the xy plane
            for i in range(4):
                r = trial.suggest_float(f'r_sp_{i}', 1.5, 4.0)
                base_phi = i * (np.pi / 2)       # 0, 90, 180, 270 deg
                dphi = trial.suggest_float(f'dphi_sp_{i}', -0.2, 0.2)
                phi = base_phi + dphi
                # nearly planar: theta ~ pi/2 (a small excursion above or below is allowed)
                theta = np.pi/2 + trial.suggest_float(f'dtheta_sp_{i}', -0.2, 0.2)
                
                x = r * np.sin(theta) * np.cos(phi)
                y = r * np.sin(theta) * np.sin(phi)
                z = r * np.cos(theta)            # for square planar this keeps z near 0
                coords.append([x, y, z])
        ...
        # -----------------------------
        # extract the structural features from the coordinates
        # -----------------------------
        features = {}
        ...
        # angles at the centre (magnetic atom at the origin): ligand-metal-ligand
        center_angles = []
        ...
        # ligand vertex angles: ligand-ligand-ligand (the BOND_df convention)
        ...
        # axis-related features (corresponding to long/short axis in ELONG_df)
        if len(coords) >= 2:
            ...
            try:
                ...
                if eigenvalues[0] > 1e-8:
                    ...
                    # fallback: derive from the bond length
        ...
        # s and delta (shape distortion and off-centring) - the same as ELONG_df
        s_val, delta_val = compute_motif_s_delta(coords)
        features['avg_s'] = s_val
        features['avg_delta'] = delta_val
        
        # -----------------------------
        # per-element and electronic-structure features
        # -----------------------------
        ...
        # delta chi, delta Z and related quantities
        features['delta_chi'] = (
        ...
        # -----------------------------
        # motif volume or area, and the packing fraction
        # -----------------------------
        motif_measure = compute_motif_measure(coords, polyhedron)
        features['avg_motif_measure'] = motif_measure
        
        # unit_cell_volume is left to Optuna (the fin_data.csv range takes precedence)
        ...
        if unit_cell_volume > 0:
            ...
            pf = 0.0
        # physically it is hard to exceed 1, so anything above is clamped to 1
        if pf > 1.0:
            pf = 1.0
        if pf < 0.0:
            pf = 0.0
        ...
        # dimension: 2 only for square planar, 3 otherwise
        ...
        # -----------------------------
        # remaining features left to Optuna
        # -----------------------------
        # p_metric, p_metric_std : ranges taken from fin_data
            # -----------------------------
    # remaining features left to Optuna
    # -----------------------------
    # p_metric, p_metric_std : ranges taken from fin_data (unchanged from the original)
        ...
        # ---- labelled/global distances and d_global_local_* ----
        # 1st distance: prefer the range taken from fin_data
        ...
        # 2nd and 3rd are optimized as the 1st plus a delta
        # (the range 0.0-0.3 is indicative; adjust it against the fin_data values if desired)
        ...
        # d_global_local_* = global_* - labelled_*  (the same definition as merge_df)
        for k in ['1st', '2nd', '3rd']:
            features[f'd_global_local_{k}'] = (
                features[f'global_{k}'] - features[f'labelled_{k}']
            )

        # the Hungarian rotation angle is also searched over a range derived from fin_data
        ...
        # store auxiliary information on the Optuna trial
